...
// Manager 进程管理器
type Manager struct {
	processes map[uint]*ResidentProcess
	mu        sync.RWMutex
...
// Register 注册常驻进程
func (pm *Manager) Register(taskID uint, cmd *exec.Cmd, stdin *StdinWrapper, sessionID string, workDir string) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
...
// Get 获取常驻进程
func (pm *Manager) Get(taskID uint) (*ResidentProcess, bool) {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
...
func (pm *Manager) UpdateLastUsed(taskID uint) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
...
// Remove 移除常驻进程（手动停止）
func (pm *Manager) Remove(taskID uint) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
...
func (pm *Manager) IsActive(taskID uint) bool {
	_, exists := pm.Get(taskID)
	return exists
}

// List 列出所有活跃的常驻进程
func (pm *Manager) List() []uint {
	pm.mu.RLock()
	defer pm.mu.RUnlock()
